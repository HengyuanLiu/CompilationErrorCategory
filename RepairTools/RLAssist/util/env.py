...
class Env_engine:
    ...
    def gui_set_cursor(self, prog_vector, cursor=None):
        ...
        if not cursor:
            if self.top_down_movement:
                ...
                cursor = self.rng.choice(xrange(len(prog_vector)))
        position = cursor + 1
    ...
    def show(self, program_id, program_vector, name_dict, name_seq, original_program_vector, use_compiler):
        ...
        err_list = err_list if use_compiler else map(lambda x: x + 1, err_list)
    ...
    def new_random_episode_with_right_actions(self, program_id, cursor_program, name_dict, name_seq, original_program, org_err_cnt, use_compiler, toy=False):
        ...
        actions = map(lambda x: self.get_action(x), action_names)
...
class Environment(Env_engine):
    def __init__(self, dataset, step_penalty, seed, GE_ratio=None, top_down_movement=True, single_delete=True,
        ...
        if single_program is not None:
            ...
            self.code_ids = {'train': train_code_ids, 'GE_train': guided_train_code_ids, 'valid': self.dataset.valid_ex.keys(),
                             'test': self.dataset.test_ex.keys(), 'real': real_data_keys, 'seeded': seeded_data_keys}
...
class load_data:
    def _deserialize(self, data_folder):
        train_ex = np.load(os.path.join(
            data_folder, 'examples-train.npy')).item()
        valid_ex = np.load(os.path.join(
            data_folder, 'examples-validation.npy')).item()
        test_ex = np.load(os.path.join(
            data_folder, 'examples-test.npy')).item()
        ...
        if load_real_test_data:
            try:
                self.real_test_data = np.load(os.path.join(
                    data_folder, 'test_real_raw.npy')).item()
            except:
                self.real_test_data = np.load(os.path.join(
                    data_folder, 'test_raw.npy')).item()
        if load_seeded_test_data:
            try:
                self.seeded_test_data = np.load(os.path.join(
                    data_folder, 'test_real_seeded.npy')).item()
            except:
                self.seeded_test_data = np.load(os.path.join(
                    data_folder, 'test_seeded.npy')).item()
        try:
            self.name_dict_store = np.load(os.path.join(
                data_folder, 'name_dict_store.npy')).item()
        except:
