...
with sqlite3.connect(iitk_db_path) as conn:
    ...
    for bin_id in range(5):
        ...
        bin_raw_test_data = np.load(os.path.join(deepfix_base_dir, 'bin_%d' % bin_id, 'test_raw_bin_%d.npy' % bin_id)).item()
        bin_seeded_test_data = np.load(os.path.join(deepfix_base_dir, 'bin_%d' % bin_id, 'test_seeded-typo_bin_%d.npy' % bin_id)).item()
