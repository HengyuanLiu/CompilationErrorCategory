...
def repairProgram(fname, predAtK):
    ...
    # print('-'*20 + '\nOriginal Code\n' + '-'*20 + '\n' + srcText)
    # print('-'*20 + '\nMACER\'s Repair\n' + '-'*20 + '\n' + predText)
    # print('\nPartially Fixed? ', 0 <= Code(predText).getNumErrors() < Code(srcText).getNumErrors())
    # print('\nCompletely Fixed? ', Code(predText).getNumErrors() == 0)
    result_dict = {}
    result_dict["P"] = 0 <= Code(predText).getNumErrors() < Code(srcText).getNumErrors()
    result_dict["C"] = Code(predText).getNumErrors() == 0
    return result_dict
